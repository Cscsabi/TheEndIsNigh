...
//Ezekben a sorokban beszédes neveket rendelünk a mágikus karakterekhez, hogy érthetőbb legyen a kód
...
double expression();			//előre létrehozzuk az expression függvényt, mert azelőtt meghívjuk, mielőtt elkészítjük
double primary();				//előre létrehozzuk az primaryfüggvényt, mert azelőtt meghívjuk, mielőtt elkészítjük

class Variable{					//létrehozza a változóhoz szükséges struktúrát
public:
	string name;
	double value;
};
...
bool is_declared(string var){		//megadja, hogy az adott változó létezik-e már
...
Token Token_stream::get()		//felkészíti használatra a műveleti jeleket, a számokat, valamint a változókat
{
	if (full)
	{
		full = false;
...
double calc_sqrt()		//négyzetgyököt számolunk
{
	char ch;
